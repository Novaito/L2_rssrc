...
void DessineUneLigneBus(TlisteStation l, LTexture mySpriteArretBus, SDL_Renderer* gRenderer){
    ...
    if (ligneBusVide(maLigneBus)){
        ...
        bool looped = false;
        while (!ligneBusVide(maLigneBus) && !looped){
...
void DessineAllLignes(Treseau *r, LTexture mySpriteArretBus, SDL_Renderer *gRenderer) {
    for (int i=0; i<getNbLigne(r); i++) {
        DessineUneLigneBus(getDepartListeStation(getNthLigne(r, i+1)), mySpriteArretBus, gRenderer);
    }
}
