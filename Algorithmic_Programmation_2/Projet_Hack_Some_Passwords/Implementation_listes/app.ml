...
(**
  ...
  Temps O(n) Car appelle de deux fonctions deux fontions en O(n)
  Espace O(1) Car appelle de deux fonctions deux fontions en O(1)
*)
let try_known_user(login, hash, list_known_user: string * string * t_known_user list): (bool * string) =
...
(**
  ...
  Temps: O(n) car toutes les fonctions appélées sont en O(n)
  Espace: O(n) car les fonctions add_to_known_hash et add_to_known_user sont en O(n)
*)
...
(**
  ...
  Temps: O(n²) car on parcours une liste d'utilisateurs a trouver donc une opération O(n), dans laquelle on appelle find_pwd_user en O(n). Ce qui donne du O(n²) au final.
  Espace: O(n)
*)
...
(**
  ...
  @return [(string * string * string) list] (nom app, login, mdp)
  Temps: O(n²) car appelle la fonction get_all_pwd pour chaque appication
  Spatiale O(n) car on appelle des fonctions en O(n) + on stocke une nouvelle liste contenant les résultats.
*)
