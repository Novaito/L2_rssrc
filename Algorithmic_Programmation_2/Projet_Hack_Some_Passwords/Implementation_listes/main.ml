...
(**
  ...
  @author Thomas Noé
  Temps: O(n²) car on appelle inject_with_app
  Espace O(n) car on appelle inject_with_app et on stocke les données des applications dans une nouvelle liste.
*)
let main() : unit = 
  let all_users_known : t_known_user list ref = ref []
  and all_hash_known : t_known_hash list ref = ref []
  ...
  |] in (
    ...
    all_users_known := make_known_user_list(all_users_depensetout) ;
